...
IMAGE_NAME="ljexport:latest"
DOCKERFILE="$PROJECT_ROOT/docker/Dockerfile"
DOCKER_CONTEXT="$PROJECT_ROOT"
SCRIPTS_DIR="$PROJECT_ROOT/docker/scripts"   # bind-mounted read-only

# ───────── (optional) always rebuild on every run ─────────
...
########################################
# 2. Bitwarden creds (unchanged logic)
########################################
if [[ -z "$LJ_USER" || -z "$LJ_PASS" ]]; then
  if command -v bw >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
    ...
    if [[ "$(bw status "${BW_FLAGS[@]}" | jq -r .status 2>/dev/null)" == "unlocked" ]]; then
